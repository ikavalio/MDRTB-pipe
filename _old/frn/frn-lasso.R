...
drug.file <- drug.files[grepl(drug, drug.files)]
...
snps.cor <- snps.cor[stats.table$mutations, stats.table$mutations]

stopifnot(lasso.possible)

frn.fit <- function(lambda, dump.to = NULL) {
  signif <- frn.find.significant(stats.table, snps.cor, lambda = lambda, 
                                 corr.threshold = cor.threshold)
  ms <- length(signif)
...
if (is.na(lambda)) {
  ...
  res <- frn.fit(lambda, dump.to = sprintf("%s/%s.lasso.signif.txt", out.path, drug))
  
  stats <- sprintf("Lambda, %f\nF1, %.3f\nTotal, %.3f\n", lambda, res[1], res[2])
  writeLines(stats, con = sprintf("%s/%s.lasso.stat.txt", out.path, drug))
}